...
RC Db::drop_table(const char *table_name) {
  ...
  std::string meta_file = std::string(table_name) + TABLE_META_SUFFIX;
  // std::string data_file = path_ + "/" + table_name + TABLE_DATA_SUFFIX;
  // Table *table = new Table();
  // table->open(meta_file.c_str(), data_file.c_str());
  ...
  LOG_DEBUG("ZD: delete table success.");
  return rc;
}
...
RC Db::open_all_tables() {
  ...
  for (const std::string &filename : table_meta_files) {
    ...
    if (opened_tables_.count(table->name()) != 0) {
      delete table;
      LOG_ERROR(
          "Duplicate table with difference file name. table=%s, the other "
          "filename=%s",
