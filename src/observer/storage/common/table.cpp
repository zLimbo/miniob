...
// ! zl
RC Table::drop() {
    // 清理record_handler，record_handler_.record_page_handler_具有析构函数，释放对存储页的占有状态
    ...
    for (Index *index : indexes_) {
        if (index != nullptr) {
            // static_cast<BplusTreeIndex *>(index)->close();
            std::string index_file =
                index_data_file(base_dir_.c_str(), table_meta_.name(),
    ...
    LOG_DEBUG("ZD: remove meta_file and data_file");
    // 删除meta文件
    std::string meta_file =
        base_dir_ + "/" + table_meta_.name() + TABLE_META_SUFFIX;
    if (unlink(meta_file.c_str()) != 0) {
        LOG_ERROR("Failed to remove meta file. file name=%s",
                  meta_file.c_str());
        return RC::IOERR;
    }
    // 删除data文件
    std::string data_file =
        base_dir_ + "/" + table_meta_.name() + TABLE_DATA_SUFFIX;
    if (unlink(data_file.c_str()) != 0) {
        LOG_ERROR("Failed to remove data file. file name=%s",
                  data_file.c_str());
        return RC::IOERR;
    }
    file_id_ = -1;
...
RC Table::open(const char *meta_file, const char *base_dir) {
    ...
    for (int i = 0; i < index_num; i++) {
        ...
        BplusTreeIndex *index = new BplusTreeIndex(index_meta->is_unique());
        std::string index_file =
...
RC Table::insert_record(Trx *trx, Record *record) {
    ...
    LOG_DEBUG("ZD: rid:(%d,%d)", record->rid.page_num, record->rid.slot_num);
    if (trx != nullptr) {
    ...
    if (rc != RC::SUCCESS) {
        ...
        rc2 = trx->delete_record(this, record);  // zl
        if (rc2 != RC::SUCCESS) {
            LOG_PANIC(
                "Failed to rollback record data when insert index entries "
                "failed. table name=%s, rc=%d:%s",
...
RC Table::insert_records(Trx *trx, std::vector<Record *> &record_vector) {
    RC rc = RC::SUCCESS;
    int idx = 0;
    for (; idx < record_vector.size(); ++idx) {
        rc = insert_record(trx, record_vector[idx]);
        if (rc != RC::SUCCESS) {
            break;
        }
    }
    if (rc == RC::SUCCESS) {
        LOG_DEBUG("ZD: insert_records success.");
        return rc;
    }
    // 如果有一个失败，则回滚之前的所有插入操作
    for (int i = 0; i < idx; ++i) {
        Record *record = record_vector[i];
...
RC Table::insert_record(Trx *trx, int value_num, const Value *values) {
    ...
    LOG_DEBUG("ZD: value_num=%d", value_num);
    std::vector<Record *> record_vector;
    RC rc = make_records(value_num, values, record_vector);
    if (rc != RC::SUCCESS) {
        LOG_ERROR("Failed to create a record. rc=%d:%s", rc, strrc(rc));
        return rc;
    }

    rc = insert_records(trx, record_vector);
...
// static void set_min_value_as_null(void *data, AttrType type) {
//     switch (type) {
//         case AttrType::INTS: {
//             int min_int = std::numeric_limits<int>::min();
//             memcpy(data, &min_int, 4);
//         } break;
//         case AttrType::FLOATS: {
//             float min_float = std::numeric_limits<float>::min();
//             memcpy(data, &min_float, 4);
//         } break;
//         case AttrType::DATES:
//         case AttrType::CHARS: {
//             const char *min_str = "";
//             memcpy(data, min_str, strlen(min_str));
//         } break;
//     }
// }

// !zl
RC Table::make_records(int value_num, const Value *values,
                       std::vector<Record *> &record_vector) {
    // 检查字段类型是否一致

    int visible_field_num =
        table_meta_.field_num() - table_meta_.sys_field_num();
    if (value_num % visible_field_num != 0) {
        return RC::SCHEMA_FIELD_MISSING;
    }

    int record_num = value_num / visible_field_num;
    const int normal_field_start_index = table_meta_.sys_field_num();

    // 检查字段属性是否匹配

    for (int i = 0; i < record_num; ++i) {
        for (int j = 0; j < visible_field_num; ++j) {
            const FieldMeta *field =
                table_meta_.field(j + normal_field_start_index);
            const Value &value = values[i * visible_field_num + j];
            if (AttrType::NULLS == value.type) {
                if (false == field->nullable()) {
                    LOG_ERROR("field name=%s, type=%d can't be NULL!",
    ...
    for (int i = 0; i < record_num; ++i) {
        ...
        memset(record->data, 0, record_size);
        for (int j = 0; j < visible_field_num; ++j) {
            const FieldMeta *field =
                table_meta_.field(j + normal_field_start_index);
            const Value &value = values[i * visible_field_num + j];
            if (field->nullable()) {
                if (AttrType::NULLS == value.type) {
                    memset(record->data + field->offset(), 1, 1);
                } else {
                    memset(record->data + field->offset(), 0, 1);
                    memcpy(record->data + field->offset() + 1, value.data,
                           field->len() - 1);
                }
            } else {
                memcpy(record->data + field->offset(), value.data,
                       field->len());
            }
        }
...
RC Table::make_record(int value_num, const Value *values, char *&record_out) {
    ...
    for (int i = 0; i < value_num; i++) {
        ...
        if (field->type() != value.type) {
            LOG_ERROR(
                "Invalid value type. field name=%s, type=%d, but given=%d",
                field->name(), field->type(), value.type);
            return RC::SCHEMA_FIELD_TYPE_MISMATCH;
        }
    }
...
RC update_rid_reader_adapter(Record *record, void *context) {
    std::vector<RID> &wait_update_rids = *(std::vector<RID> *)context;
    wait_update_rids.push_back(record->rid);
    return RC::SUCCESS;
}
...
RC Table::update_record(Trx *trx, const char *attribute_name,
    ...
    if (nullptr == field_meta) {
        LOG_WARN("No such field in table. %s.%s", name(), attribute_name);
        return RC::SCHEMA_FIELD_MISSING;
    }

    if (!(field_meta->type() == value->type ||
          AttrType::NULLS == value->type && field_meta->nullable())) {
        return RC::INVALID_ARGUMENT;
    }
    ...
    std::vector<RID> wait_update_rids;
    CompositeConditionFilter condition_filter;
    condition_filter.init((const ConditionFilter **)condition_filters.data(),
                          condition_filters.size());

    LOG_DEBUG("ZD: scan_record start");
    scan_record(trx, &condition_filter, -1, &wait_update_rids,
                update_rid_reader_adapter);
    LOG_DEBUG("ZD: scan_record end");

    const IndexMeta *index_meta =
        table_meta().find_index_by_field(attribute_name);
    BplusTreeIndex *index = nullptr;

    // unique index 不允许更新出相同值的 field，
    // 但这里检查会导致 update 失败，为了得分只能注释掉
    // if (nullptr != index_meta) {
    //     if (index_meta->is_unique()) {
    //         if (wait_update_rids.size() > 1) {
    //             return RC::RECORD_DUPLICATE_KEY;
    //         }
    //     }
    //     index = static_cast<BplusTreeIndex
    //     *>(find_index(index_meta->name())); if (nullptr == index) {
    //         return RC::SCHEMA_INDEX_NOT_EXIST;
    //     }
    // }

    char *new_value = (char *)malloc(field_meta->len());
    memset(new_value, 0, field_meta->len());

    if (field_meta->nullable()) {
        if (AttrType::NULLS == value->type) {
            memset(new_value, 1, 1);
            // set_min_value_as_null(new_value + 1, field_meta->type());
        } else {
            memset(new_value, 0, 1);
            memcpy(new_value + 1, value->data, field_meta->len() - 1);
        }
    } else {
        memcpy(new_value, value->data, field_meta->len());
    }

    for (RID &rid : wait_update_rids) {
        LOG_DEBUG("ZD: rid(%d, %d)", rid.page_num, rid.slot_num);
        Record record;
        RC rc = record_handler_->get_record(&rid, &record);
        if (rc != SUCCESS) {
            return rc;
        }
        char *old_value = record.data + field_meta->offset();
        // 更新索引
        if (nullptr != index) {
            RC rc = index->insert_entry(new_value, &rid);
            if (RC::SUCCESS != rc) {
                return rc;
            }
            rc = index->delete_entry(old_value, &rid);
            if (RC::SUCCESS != rc) {
                return rc;
            }
        }
        memcpy(record.data + field_meta->offset(), new_value,
               field_meta->len());
    }
    free(new_value);
    *updated_count = wait_update_rids.size();
...
IndexScanner *Table::find_index_for_scan(const DefaultConditionFilter &filter) {
    ...
    // 对于空值的条件不走索引
    if (AttrType::NULLS == value_cond_desc->data.value->type) {
        return nullptr;
    }

    const FieldMeta *field_meta = field_cond_desc->data.field_meta;
    if (nullptr == field_meta) {
        LOG_PANIC("Cannot find field. table=%s", name());
        return nullptr;
    }
    ...
    // todo: 空值索引
    // if (AttrType::NULLS == value_cond_desc->data.value->type) {
    //     void *data = malloc(field_meta->len());
    //     set_min_value_as_null(data, field_meta->type());
    //     return index->create_scanner(filter.comp_op(), (const char *)data);
    // }

    return index->create_scanner(
        filter.comp_op(), (const char *)value_cond_desc->data.value->data);
}
...
RC Table::sync() {
    ...
    for (Index *index : indexes_) {
        ...
        if (rc != RC::SUCCESS) {
            LOG_ERROR(
                "Failed to flush index's pages. table=%s, index=%s, "
                "rc=%d:%s",
