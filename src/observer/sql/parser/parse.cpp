...
void value_init_date(Value *value, const char *v) {
    LOG_DEBUG("ZD: value->data=%s", v);
    value->type = DATES;
    // 日期格式规范化 yymm-MM-dd
    char *date = (char *)malloc(12);
    memcpy(date, v, 5);
    int left_pos = 5, right_pos = 5;
    if (v[right_pos + 1] != '-') {
        date[left_pos++] = v[right_pos++];
    } else {
        date[left_pos++] = '0';
    }
    date[left_pos++] = v[right_pos++];
    ...
    date[left_pos] = '\0';
    value->data = date;
    LOG_DEBUG("ZD: value->data:%s", value->data);
}
...
void selects_append_aggregate(Selects *selects, const char *aggregate_name) {
    char *aggregate_name_ = strdup(aggregate_name);
    // 转化为小写
    for (char *p = aggregate_name_; *p != '\0'; ++p) {
        *p = tolower(*p);
    }
    selects->aggregates[selects->aggregate_num++] = aggregate_name_;
}
...
void selects_append_order(Selects *selects, const char *relation_name,
                          const char *attribute_name, int is_desc) {
    size_t index = selects->order_num++;
    relation_attr_init(&(selects->orders[index].attr), relation_name,
                       attribute_name);
    selects->orders[index].is_desc = is_desc;
}

void selects_append_join(Selects *selects, const char *relation_name,
                         Condition *condition) {
    size_t index = selects->join_num++;
    selects->joins[index].relation_name = strdup(relation_name);
    selects->joins[index].condition = *condition;
}

void selects_destroy(Selects *selects) {
    for (size_t i = 0; i < selects->aggregate_num; ++i) {
        free(selects->aggregates[i]);
        selects->aggregates[i] = NULL;
    }
    ...
    for (size_t i = 0; i < selects->order_num; ++i) {
        free(selects->orders[i].attr.attribute_name);
        free(selects->orders[i].attr.relation_name);
        selects->orders[i].is_desc = 0;
    }
    selects->order_num = 0;

    for (size_t i = 0; i < selects->join_num; ++i) {
        free(selects->joins[i].relation_name);
        condition_destroy(&selects->joins[i].condition);
    }
    selects->join_num = 0;
}
...
void set_index_unique(CreateIndex *create_index, int flag) {
    create_index->is_unique = flag;
}
