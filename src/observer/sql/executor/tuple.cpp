...
// !zl
Tuple::Tuple(const Tuple &other) : values_(other.values_) {
...
void Tuple::add(const char *s) { add(new StringValue(s)); }
...
void Tuple::add(const std::vector<std::shared_ptr<TupleValue>> &other) {
    for (const std::shared_ptr<TupleValue> &value : other) {
        add(value);
    }
}
...
std::string TupleField::to_string() const {
    std::string result;
    if (aggregate_name_.empty()) {
        result = table_name_ + "." + field_name_ + std::to_string(type_);
    } else {
        result = aggregate_name_ + "(" + field_name_ + ")";
    }
    return result;
}
...
void TupleSchema::add(AttrType type, const char *table_name,
                      const char *field_name, const char *aggregate_name) {
...
void TupleSchema::print(std::ostream &os, bool is_tables) const {
    if (fields_.empty()) {
        os << "No schema";
        return;
    }
    ...
    auto tuple_field_print = [&](const TupleField &tuple_field,
                                 const std::string &sep) {
        const std::string &aggregate_name = tuple_field.get_aggregate_name();
        if (!aggregate_name.empty()) {
            os << aggregate_name << "(";
        }
        if ((table_names.size() > 1 || is_tables) &&
            0 != strcmp("", tuple_field.table_name())) {
            os << tuple_field.table_name() << ".";
        }
        os << tuple_field.field_name();
        if (!aggregate_name.empty()) {
            os << ")";
        }
        os << sep;
    ...
    for (std::vector<TupleField>::const_iterator iter = fields_.begin(),
         ...
         iter != end; ++iter) {
        tuple_field_print(*iter, " | ");
    }
    tuple_field_print(fields_.back(), "\n");
}
...
void TupleSet::print(std::ostream &os, bool is_tables) const {
    if (schema_.fields().empty()) {
        LOG_WARN("Got empty schema");
        return;
    }

    schema_.print(os, is_tables);
...
void TupleSet::set_tuples(std::vector<Tuple> &&tuples) {
...
void TupleRecordConverter::add_record(const char *record) {
    ...
    for (const TupleField &field : schema.fields()) {
        ...
        if (field_meta->nullable()) {
            bool is_null = *(bool *)(record + field_meta->offset());
            if (is_null) {
                tuple.add(new NullValue());
                continue;
            }
        }
        int value_offset = field_meta->offset() + field_meta->nullable();
        switch (field_meta->type()) {
            case INTS: {
                int value = *(int *)(record + value_offset);
                tuple.add(value);
            } break;
            case FLOATS: {
                float value = *(float *)(record + value_offset);
                tuple.add(value);
            } break;
            case DATES:
            case CHARS: {
                const char *s = record + value_offset;  // 现在当做Cstring来处理
                tuple.add(s, strlen(s));
            } break;
            default: {
                LOG_PANIC("Unsupported field type. type=%d",
                          field_meta->type());
            }
        }
    }
